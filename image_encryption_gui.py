...
# Function to check integrity of decrypted files
def check_integrity(original_folder, decrypted_folder):
    for root, dirs, files in os.walk(original_folder):
        for file_name in files:
            original_path = os.path.join(root, file_name)
            decrypted_path = os.path.join(decrypted_folder, file_name)
...
def validate_and_process(operation_var, folder_var, key_var, mode_var):
    ...
    if operation == 'Encrypt':
        ...
        if check_integrity(folder_path, folder_path):
...
execute_button = tk.Button(root, text="Execute", command=lambda: validate_and_process(operation_var, folder_var, key_var, mode_var))
