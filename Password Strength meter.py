...
def update_password_strength(password_var, strength_label):
    password = password_var.get()
    if len(password) >= 16:
        strength_label.config(text=f"Password Strength: {len(password)}/16 (Strong)")
    else:
        strength_label.config(text=f"Password Strength: {len(password)}/16 (Weak)")
...
strength_label = tk.Label(root, text="", fg="blue")
strength_label.grid(row=6, column=0, columnspan=2, padx=10, pady=5)

password_var.trace("w", lambda *args: update_password_strength(password_var, strength_label))
...
execute_button.grid(row=7, column=0, columnspan=2, padx=10, pady=10)
