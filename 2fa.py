...
def validate_and_process(operation_var, folder_var, key_var, mode_var, signature_var, status_label, progress_bar, token_var):
    operation = operation_var.get()
    folder_path = folder_var.get()
    key = key_var.get()
    mode = mode_var.get()
    signature = signature_var.get()
    token = token_var.get()  # Get the TOTP token entered by the user
...
token_label = tk.Label(root, text="Enter TOTP:")
...
execute_button = tk.Button(root, text="Execute", command=lambda: validate_and_process(operation_var, folder_var, key_var, mode_var, signature_var, status_label, progress_bar, token_var))
...
status_label.grid(row=8, column=0, columnspan=2, padx=10, pady=5)
